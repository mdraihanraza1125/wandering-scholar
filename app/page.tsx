...
const MountainLogo = () => <svg viewBox="0 0 100 35" width="85" height="30" fill="#111" className="mb-1"><path d="M25 15 L0 35 L50 35 Z" opacity="0.9" /><path d="M75 12 L50 35 L100 35 Z" opacity="0.9" /><path d="M50 0 L20 35 L80 35 Z" /></svg>;
...
export default function Home() {
  ...
  const categories = [
    ...
    { title: 'POETRY', slug: 'poetry', desc: 'Nature and silent moments.', icon: '✒️' },
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] text-gray-800 font-serif">
      ...
      <header className="bg-white py-5 px-6 lg:px-10 border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <Link href="/" className="flex flex-col items-center">
            <MountainLogo />
            <h1 className="text-lg font-bold tracking-wider text-center">THE WANDERING SCHOLAR</h1>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-x-6 text-[10px] font-sans tracking-widest font-bold">
            ...
            <Link href="/poetry" className="hover:text-[#c66b1a] transition">POETRY</Link>
      ...
      {/* SEARCH BAR (Dropdown) */}
      {isSearchOpen && (
        <div className="sticky top-[85px] left-0 w-full bg-white border-b border-gray-100 py-4 px-6 z-40 shadow-sm flex justify-center">
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 flex justify-end">
          <div className="w-64 bg-white h-full shadow-2xl p-6 flex flex-col font-sans">
            ...
            <nav className="flex flex-col gap-6 text-xs font-bold tracking-widest uppercase">
              ...
              <Link href="/poetry" onClick={() => setIsMobileMenuOpen(false)} className="border-b pb-2">POETRY</Link>
      ...
      <section className="relative h-[500px] bg-gray-900 overflow-hidden flex items-center justify-center">
        ...
        <div className="relative text-center text-white p-6">
          ...
          <p className="font-sans max-w-2xl mx-auto opacity-90 leading-relaxed text-sm md:text-base">A digital journal of travels, thoughts, history, poetry, and stories.</p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto py-10 md:py-20 px-6 grid grid-cols-2 md:grid-cols-5 gap-4">
        {categories.map((cat, i) => (
          <Link href={`/${cat.slug}`} key={i} className="bg-[#1c1c1c] text-white p-6 text-center min-h-[160px] md:min-h-[200px] flex flex-col items-center justify-center cursor-pointer hover:bg-[#c66b1a] transition duration-300">
            ...
            <h4 className="text-[10px] font-bold tracking-widest uppercase mb-2">{cat.title}</h4>
          </Link>
        ))}
        <Link href="/gallery" className="bg-[#1c1c1c] text-white p-6 text-center min-h-[160px] md:min-h-[200px] flex flex-col items-center justify-center cursor-pointer hover:bg-[#c66b1a] transition duration-300 col-span-2 md:col-span-1">
          ...
          <h4 className="text-[10px] font-bold tracking-widest uppercase mb-2">PHOTO GALLERY</h4>
        </Link>
      </section>
      ...
      <section className="bg-[#f4f2ee] py-20 px-6 md:px-8 border-t border-b border-gray-200">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12">
          <div className="w-40 h-40 md:w-48 md:h-48 shrink-0">
            {settings?.avatar_url ? <img src={settings.avatar_url} className="w-full h-full object-cover rounded-full shadow-lg border-4 border-white" /> : <div className="w-full h-full rounded-full bg-gray-300 flex items-center justify-center text-5xl">👤</div>}
          </div>
      ...
      <footer className="bg-[#111] text-gray-500 py-16 text-center font-sans"><p className="text-[10px] tracking-widest">© {new Date().getFullYear()} THE WANDERING SCHOLAR. ALL RIGHTS RESERVED.</p></footer>
    </div>
  );
}